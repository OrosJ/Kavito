...
const controllers = await import(
  "../../../controllers/ProjectProductController.js"
);
const {
  getProjectProduct,
...
const ProjectProductModels = await import(
  "../../../models/ProjectProductModel.js"
);
...
describe("ProjectProductController", () => {
  ...
  describe("getProjectProduct", () => {
    test("debe retornar un producto de proyecto por ID", async () => {
      ...
      // Verificaciones
      expect(ProjectProduct.findByPk).toHaveBeenCalledWith(
        "1",
  ...
  describe("getHistory", () => {
    ...
    test("debe manejar errores al obtener historial", async () => {
      ...
      // Mock error
      ProjectProductHistory.findAll.mockRejectedValue(
  ...
  describe("reserveProjectProduct", () => {
    test("debe reservar productos correctamente", async () => {
      ...
      const mockProjectProduct = {
        ...
        productItem: {
          // AÑADIR productItem aquí para simular la relación incluida
          id: 1,
          descripcion: "Producto 1",
          cantidad: 10,
          cantidad_reservada: 0,
...
/*       expect(ProductModel.findByPk).toHaveBeenCalledWith(
      ...
      expect(mockProjectProduct.productItem.increment).toHaveBeenCalledWith(
        "cantidad_reservada",
        expect.objectContaining({
          by: 3,
          transaction: mockTransaction,
      ...
      expect(mockProjectProduct.increment).toHaveBeenCalledWith(
        "cantidad_reservada",
        expect.objectContaining({
          by: 3,
          transaction: mockTransaction,
    ...
    test("debe rechazar reserva con stock insuficiente", async () => {
      ...
      const mockProjectProduct = {
        ...
        cantidad_reservada: 0,
        estado: "PENDIENTE",
        productItem: { // AÑADIR productItem para simular la relación
          id: 1,
          descripcion: "Producto 1",
          cantidad: 10,
          cantidad_reservada: 0,
...
/*       const mockProduct = {
  ...
  describe("deliverProjectProduct", () => {
    test("debe entregar productos y generar salida de inventario", async () => {
      ...
      // Verificaciones
      expect(db.transaction).toHaveBeenCalled();
      expect(ProjectProduct.findOne).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: "1" },
          include: expect.any(Array),
          transaction: mockTransaction,
